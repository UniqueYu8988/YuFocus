...
export function createRuntimePaths(options: RuntimePathOptions): RuntimePaths {
  ...
  return {
    ...
    runtimeLogPath: path.join(options.userDataRoot, '.shijie-focus-runtime.log'),
...
export function createSettingsDefaultsContext(options: {
  ...
  return {
    ...
    legacyOutputRoots: [
      path.resolve(options.devProjectRoot, 'desktop', 'release', 'output'),
      path.resolve(options.devProjectRoot, 'desktop', 'output'),
    ],
