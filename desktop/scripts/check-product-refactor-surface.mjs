...
assert.doesNotMatch(workspace, /WorkspaceDialogs|KnowledgeBriefDialog|MarkdownRenderer|readTextFile\(/, '软件内部 Markdown 查看器入口不应再挂载。')
assert.match(workspace, /openPath\(targetPath\)/, 'Markdown 文档应交给系统外部应用打开。')
assert.ok(!exists('src/ui/panels/workspace/WorkspaceDialogs.tsx'), '旧内部 Markdown 查看器包装组件应删除。')
assert.ok(!exists('src/ui/components/KnowledgeBriefDialog.tsx'), '旧内部 Markdown 查看器组件应删除。')
assert.ok(!exists('src/ui/components/MarkdownRenderer.tsx'), '旧内部 Markdown 渲染器组件应删除。')
