...
export function WorkbenchQueuePanel({
  ...
  onBuildSummary: (record: MaterialInventoryRecord) => void
...
function WorkbenchQueueRow({
  ...
  const canBuildEditorialSummary = Boolean(
    record &&
    !record.editorialSummaryExists &&
    (record.notebooklmExists || record.rawTranscriptExists),
  )
  return (
    <div
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
          ...
          {record ? (
            ...
            ) : canBuildEditorialSummary ? (
              <Button
                variant="ghost"
                size="icon"
                className="size-7 rounded-lg text-foreground/68 hover:bg-emerald-400/[0.10] hover:text-emerald-100"
                disabled={editorialSummaryBuilding || distillRequestState === 'loading'}
                onClick={(event) => {
                  event.stopPropagation()
                  onBuildSummary(record)
                }}
                aria-label={editorialSummaryBuilding ? '正在制作文稿' : '制作文稿'}
                title={editorialSummaryBuilding ? '正在制作文稿' : '制作文稿'}
              >
                {editorialSummaryBuilding ? <LoaderCircle size={13} className="animate-spin" /> : <FileText size={13} />}
              </Button>
            ) : null
