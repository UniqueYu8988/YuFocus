...
import {
  ...
  type ArchiveStats,
} from './ArchivePaneParts'
import type { KnowledgeSourceFilter } from './KnowledgePaneParts'
...
export function filterArchiveMaterialRecords(options: {
  ...
  return options.records.filter((record) => {
    ...
    if (options.shelfFilter === 'pushed') return Boolean(record.emailPushedAt)
...
export function buildArchiveStats(records: MaterialInventoryRecord[]): ArchiveStats {
  ...
  return {
    ...
    pushed: records.filter((record) => record.emailPushedAt).length,
...
export function buildArchiveFilters(records: MaterialInventoryRecord[], pinnedSources: PinnedBilibiliSource[]): ArchiveFilterItem[] {
  ...
  return [
    ...
    { id: 'pushed' as const, label: '已推送', count: records.filter((record) => record.emailPushedAt).length },
