...
function buildCourseLesson(section: MarkdownSection, order: number): CourseNode {
  return {
    ...
    teacher_ready_content: {
      ...
      display_hints: ['learning_notes', 'chapter_reading'],
    ...
    knowledge: {
      ...
      common_mistakes: ['跳读后只留下零散信息，没有形成章节主线。'],
    },
    children: [],
    assets: [],
    gaps: [],
...
function buildCourseChapter(chapter: MarkdownChapter, index: number): CourseNode {
  return {
    ...
    knowledge: {
      ...
      common_mistakes: ['只读散点，没有把本章放回全文主线。'],
    },
...
export function buildLearningNotesStudyPackage(options: BuildLearningNotesStudyPackageOptions): StudyPackageCompat {
  ...
  const courseTitle = extractCourseTitle(cleanBrief, options.title || '未命名学习笔记')
  const chapters = splitMarkdownIntoChapters(cleanBrief)
  ...
  // The learning desk still consumes the historical CoursePackage object shape.
  // External metadata names this artifact as a study package created from notes.
  return {
    ...
    source: {
      ...
      notes: `由学习笔记学习包导入学习台。材料包：${options.materialPath}`,
    },
    course: {
      title: courseTitle,
      subtitle: '学习笔记学习包',
      overall_goal: '按章节阅读、复述并建立视频内容中的知识联系。',
