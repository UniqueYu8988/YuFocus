...
底座升级顺序是：先做 `validator + ready 分层骨架`，再做 `source_index + trace map`、`read-only quality audit`、`golden eval + 300k synthetic test`。当前测试如果再次出现假 ready，不再优先调提示词，应优先补强底座证据链。
