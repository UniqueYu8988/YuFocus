...
- `validation_contract.json` 是每个材料包的机器可读验证合同。新包使用项目 profile 的 resolved snapshot；旧包缺失时由 validator 自动补 legacy contract。第一阶段 v8.1 先强制正文厚度、H3 长度、trace、干净正文和 ready 分层；`learning_page_plans`、`required_source_cards`、`published_claims` 在后续 strict contract 中逐步升为 required。
...
底座升级顺序是：先做 `validation_contract + CLI validator + ready 分层骨架`，再做 `source_index + trace map`、`read-only quality audit`、`golden eval + 300k synthetic test`，随后逐步引入 `learning_page_plans`、`candidate/required source_cards` 和 `published_claims`。当前测试如果再次出现假 ready，不再优先调提示词，应优先补强底座证据链。
