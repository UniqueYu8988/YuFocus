...
- `validation_contract.json` 是每个材料包的机器可读验证合同。新包使用项目 profile 的 resolved snapshot；旧包缺失时由 validator 自动补 legacy contract。v8.3 新包默认 content-specific strict：正文厚度、H3 长度、trace、干净正文、ready 分层、`learning_page_plans`、candidate/required `source_cards` 和 `published_claims` 都会进入 deterministic gate；validator 还会扣除通用学习话术、重复复盘模板和跨学习单位高相似句群，只把本节专属学科内容计入有效厚度。
...
这些是充分性原则，不是固定模板。Codex 可以根据内容自由选择块形态，但二次编辑时必须确认：高价值小节没有“说一半就停”，正文没有变成单调长段落，也没有出现“完成方式、验收方式、能复述才算完成”这类进度提示。长材料正文加厚时，应优先补组织来源、发生位置、镜下特点、机制链、分型边界、题干触发词、例外和鉴别点；不要用每节都相同的复盘抓手、答题路径或考前自问来通过字数门槛。
...
第三阶段开始，strict validator 会把旁路证据和 `learning_notes.md` 的真实学习单位交叉核对：`target_heading` 必须匹配最终 Markdown 标题；每个最终学习单位都要有 page plan 和 published claim；claim 引用的 required source card 应列入同一学习单位的 page plan。这样可以挡住“证据文件齐备，但其实对应旧标题或另一个草稿”的假 ready。
