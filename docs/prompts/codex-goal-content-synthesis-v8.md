...
- `content_draft/work/topic_inventory.json`
- `content_draft/work/source_cards/candidates/*.jsonl`
...
候选 source cards 是从原文抽出的可用素材，不等于最终必须使用的证据。每条候选卡至少写：

- `card_id`
- `source_index_entry_ids` 或 `block_ids`
- `excerpt` 或 `lock_snapshot_hash`
- `topic_ids`
- `why_candidate`
...
- `content_draft/work/section_dossiers/*.md`
- `content_draft/work/source_cards/required/*.jsonl`
...
本阶段要从候选 source cards 中冻结 required source cards。required card 是最终正文若要发布关键判断时必须可追溯的证据卡，不是所有原文片段。每条 required card 至少写：
...
- `content_draft/work/learning_page_plans/*.json`
- `content_draft/work/published_claims/*.jsonl`
- `content_draft/learning_notes.md`
...
每写一个可打开学习单位，就同步写一个学习页计划和若干 published claims。学习页计划是旁路文件，不进入学生正文；它说明这个学习单位要解决的问题、覆盖的节点、预计使用的 required source cards 和内容槽位。`target_heading` 必须和 `learning_notes.md` 里的最终 `##` 或 `###` 标题一致，包括编号和标题文字；如果最终标题改了，旁路文件也要同步改。

published claim 是最终正文里已经发布的可核查判断，至少写：
...
如果某个 claim 找不到 required source card，不要把它当成已发布结论；可以改成保守表述，或回到 dossier 阶段补 required card。每个最终学习单位至少要有对应 page plan 和 published claim；claim 引用的 required source card 应已列入同一个学习单位的 page plan。
...
- 已在 `content_draft/work/published_claims/` 中记录，并引用真实 required source cards。
...
- 对 strict contract，`learning_page_plans`、`source_cards/required` 和 `published_claims` 不是可选附件。缺失或不能解析时，validator 会保持 `pipeline_ready=false`。
- strict validator 会交叉检查这些旁路文件是否真的对应最终正文标题。不要在旁路文件里沿用旧标题、临时标题或另一个草稿的标题。
