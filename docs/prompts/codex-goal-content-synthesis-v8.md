...
每写一个可打开学习单位，就同步写一个学习页计划和若干 published claims。学习页计划是旁路文件，不进入学生正文；它说明这个学习单位要解决的问题、覆盖的节点、预计使用的 required source cards 和内容槽位。published claim 是最终正文里已经发布的可核查判断，至少写：
...
如果某个 claim 找不到 required source card，不要把它当成已发布结论；可以改成保守表述，或回到 dossier 阶段补 required card。
